...
function M.setup(opts)
  local config_module = require 'dracula-recharged.config'
...
function M.load()
  ...
  vim.g.colors_name = 'dracula-recharged'

  local config = require('dracula-recharged.config').get()
  local colors = require('dracula-recharged.colors').setup(config)

  if config.terminal_colors then
    M.set_terminal_colors(colors)
  end

  local highlights = require('dracula-recharged.highlights').setup(colors, config)
  require('dracula-recharged.highlights').apply(highlights)
end
...
function M.get_colors(opts)
  ...
  return require('dracula-recharged.colors').setup(config)
end
